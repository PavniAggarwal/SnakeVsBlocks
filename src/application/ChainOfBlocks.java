...
public class ChainOfBlocks extends Token{
	...
	public void Hit(Snake s,AnchorPane AP,ArrayList<Transition> transitions) {
		for(int i=0;i<lengthOfChain;i++)
		{
			if(s.getId().getLayoutX()>=chain.get(i).getId().getLayoutX()-22.5 && s.getId().getLayoutX()<=chain.get(i).getId().getLayoutX()+22.5)
			{
				if(s.getLength()>=chain.get(i).getValue())
				{
					chain.get(i).Hit(s,AP,transitions);
	...
	public void Destroy(Snake s) {
		...
		chain.get(0).getAnimation().setOnFinished((e)->{
			chain = null;
			lengthOfChain = 0;
		});
	}
